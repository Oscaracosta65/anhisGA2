 ...
 *
 * Game:          GA2
 * Main balls:    5 (first–fifth), range 01–42
 * Bonus ball:    None (sixth column queried for completeness but no separate pool)
 * Archive route: /lottery-archives-pick5
 *
...
if (isset($stateName, $gName) && (string) $stateName !== '' && (string) $gName !== '') {
    ...
    $doc->setMetaData(
        'description',
        'Verified draw results, frequency analysis, recency data, and hot and quiet number insights for '
        . (string) $stateName . ' ' . (string) $gName
        . '. Powered by LottoExpert analytical intelligence.'
    );
}
...
/**
 * Previous occurrence lookup — searches all six positions (GA2 game logic).
 */
...
/* ----------------------------------------------------------------
 * Frequency analysis
 * All six DB columns are searched per GA2 game logic, but counted
 * at most once per draw to avoid inflated counts.
 * ---------------------------------------------------------------- */
...
foreach ($rowsMain as $idx => $row) {
    ...
    foreach ($balls as $ball) {
        if ($ball === '' || !isset($mainCounts[$ball]) || in_array($ball, $seenInDraw, true)) {
            continue;
        }
        $mainCounts[$ball]++;
        $seenInDraw[] = $ball;
...
foreach ($window50 as $row) {
    ...
    foreach (['first', 'second', 'third', 'fourth', 'fifth', 'sixth'] as $col) {
        ...
        if ($ball !== '' && isset($counts50[$ball]) && !in_array($ball, $seenInDraw, true)) {
            $counts50[$ball]++;
            $seenInDraw[] = $ball;
        }
    }
...
foreach ($window300 as $row) {
    ...
    foreach (['first', 'second', 'third', 'fourth', 'fifth', 'sixth'] as $col) {
        $ball = trim((string) ($row[$col] ?? ''));
        if ($ball !== '' && isset($counts300[$ball]) && !in_array($ball, $seenInDraw, true)) {
            $counts300[$ball]++;
            $seenInDraw[] = $ball;
        }
    }
...
/* ----------------------------------------------------------------
 * Draw history rows for current draw context
 * ---------------------------------------------------------------- */
$drawHistoryRows = [];
 ...
 * ---------------------------------------------------------------- */
$structuredName = '';
$structuredDesc = 'Verified draw results, frequency analysis, and number pattern data.';
if (isset($stateName, $gName) && (string) $stateName !== '') {
    $structuredName = htmlspecialchars((string) $stateName . ' ' . (string) $gName . ' Results Intelligence', ENT_QUOTES, 'UTF-8');
    $structuredDesc = 'Authoritative frequency, recency, and draw history analysis for ' . (string) $stateName . ' ' . (string) $gName . '.';
}
$jsonLdPage = json_encode([
    '@context'    => 'https://schema.org',
    '@type'       => 'WebPage',
    'name'        => $structuredName !== '' ? $structuredName : 'Lottery Results Intelligence',
    'description' => $structuredDesc,
    'url'         => $canonicalNoQuery,
    'inLanguage'  => 'en',
], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
?>
<script type="application/ld+json"><?php echo $jsonLdPage !== false ? $jsonLdPage : '{}'; ?></script>
<style>
:root{
  --skai-blue:#1C66FF;
  --deep-navy:#0A1A33;
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=GA2">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=GA2">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=GA2">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=GA2">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=GA2">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;<?php echo (int) $mainBallMax; ?>. The right panel shows the recency distribution for each value in the same window.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo (int) $mainBallMax; ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--ember">
            ...
            <span class="skai-card-sub">Distance since last appearance for each main number (01&ndash;<?php echo (int) $mainBallMax; ?>)</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame skai-chart-frame--tall">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. Frequency counts how many draws contained each value. Recency shows how many draws have passed since each value last appeared.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Adjust the analysis window below to reload counts and recency for a different draw range.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo (int) $mainBallMax; ?></span>
        </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=GA2">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=GA2">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=GA2">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=GA2">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=GA2">Heatmap Analysis</a>
...
(function () {
  ...
  function renderCharts() {
    if (!window.Chart || chartsInit) { return; }
    chartsInit = true;

    var topCtx     = safeGetContext('topActiveChart');
    var quietCtx   = safeGetContext('quietChart');
    var mainCtx    = safeGetContext('fullMainChart');
    var recencyCtx = safeGetContext('recencyChart');
